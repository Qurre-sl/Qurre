...
namespace Qurre.API.Classification.Player
{
    public sealed class Inventory
    {
        ...
        {
            get => (Dictionary<ushort, Item>)Base.UserInventory.Items.Select(item => new KeyValuePair<ushort, Item>(item.Key, Item.Get(item.Value)));
            set
            {
                if (value == null)
                {
        ...
        {
            while (Base.UserInventory.Items.Count > 0) Base.ServerDropItem(Base.UserInventory.Items.First().Key);
        }
        ...
        public void AddItem(Item item, int amount)
        {
            ...
            for (var i = 0; i < amount; i++) AddItem(item.Base);
        }

        public Item AddItem(ItemType itemType)
        {
            var item = Item.Get(Base.ServerAddItem(itemType));
            ...
            if (item is Gun gun)
            {
                ...
                gun.Base.Status = new (gun.MaxAmmo, status, gun.Base.GetCurrentAttachmentsCode());
            }
        ...
        public void AddItem(ItemType itemType, int amount)
        {
            ...
            for (var i = 0; i < amount; i++) AddItem(itemType);
        }
