...
static public class Extensions
{
	...
	#region Player.Get
	static public IEnumerable<Player> GetPlayer(this Team team)
		=> Player.List.Where(player => player.RoleInformation.Team == team);

	static public IEnumerable<Player> GetPlayer(this RoleTypeId role)
		=> Player.List.Where(player => player.RoleInformation.Role == role);
	...
	{
		if (Internal.Fields.Player.IDs.TryGetValue(playerId, out var _pl))
			return _pl;
	...
	{
		if (handler is not AttackerDamageHandler adh)
			return null;

		return adh.Attacker.Hub.GetPlayer();
	}
	...
	{
		if (DamagesCached.TryGetValue(handler, out var damageType))
			return damageType;
	...
	{
		if (Prefabs.Targets.TryGetValue(prefab, out var target))
			return target;
	...
	static public MapGeneration.Distributors.Locker GetPrefab(this LockerPrefabs prefab)
	{
		...
		if (Prefabs.Lockers.TryGetValue(prefab, out var locker))
			return locker;
	...
	#region GetRoom
	static public Room GetRoom(this RoomName type)
		=> Map.Rooms.FirstOrDefault(x => x.RoomName == type);

	static public Room GetRoom(this RoomType type)
		=> Map.Rooms.FirstOrDefault(x => x.Type == type);

	static public Room GetRoom(this RoomIdentifier identifier)
		=> Map.Rooms.FirstOrDefault(x => x.Identifier == identifier);
	#endregion


	#region GetTesla
	static public Tesla GetTesla(this TeslaGate teslaGate)
		=> Map.Teslas.FirstOrDefault(x => x.GameObject == teslaGate.gameObject);

	static public Tesla GetTesla(this GameObject gameObject)
		=> Map.Teslas.FirstOrDefault(x => x.GameObject == gameObject);
	#endregion


	#region GetGenerator
	static public Generator GetGenerator(this GameObject gameObject)
		=> Map.Generators.FirstOrDefault(x => x.GameObject == gameObject);

	static public Generator GetGenerator(this Scp079Generator generator079)
		=> Map.Generators.FirstOrDefault(x => x.GameObject == generator079.gameObject);
	#endregion


	#region GetLift
	static public Lift GetLift(this ElevatorChamber elevator)
		=> Map.Lifts.FirstOrDefault(x => x.Elevator == elevator);

	static public Lift GetLift(this Vector3 position)
		=> Map.Lifts.FirstOrDefault(x => x.Bounds.Contains(position));
	#endregion


	#region GetLocker
	static public Controllers.Locker GetLocker(this MapGeneration.Distributors.Locker locker)
		=> Map.Lockers.FirstOrDefault(x => x._locker == locker);
	#endregion


	#region GetDoor
	static public Door GetDoor(this DoorVariant variant)
		=> Map.Doors.FirstOrDefault(x => x.DoorVariant == variant);

	static public Door GetDoor(this DoorType type)
		=> Map.Doors.FirstOrDefault(x => x.Type == type);
	#endregion


	#region GetRagdoll
	static public Ragdoll GetRagdoll(this BasicRagdoll basic)
		=> Map.Ragdolls.FirstOrDefault(x => x.ragdoll == basic);
	#endregion


	#region GetWorkStation
	static public WorkStation GetWorkStation(this WorkstationController controller)
		=> Map.WorkStations.FirstOrDefault(x => x.Controller == controller);
	#endregion
}