...
namespace Qurre.Internal.Patches.Alpha
{
    ...
    internal static class UnlockPanel
    {
        ...
        private static IEnumerable<CodeInstruction> Call(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            int index = list.FindIndex(ins => ins.opcode == OpCodes.Call && ins.operand is not null && ins.operand is MethodBase methodBase && methodBase.Name == nameof(PlayerInteract.OnInteract));
            ...
            list.InsertRange(
                index - 1, new[]
                {
                    ...
                    new (OpCodes.Ldfld, AccessTools.Field(typeof(PlayerInteract), nameof(PlayerInteract._hub))), // ReferenceHub
