    ...
    internal static class LocalReport
    {
        ...
        private static IEnumerable<CodeInstruction> Call(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            for (var i = 0; i < list.Count; i++)
            {
                ...
                if (index < 0 && ins.opcode == OpCodes.Ldarg_S && $"{ins.operand}" == "4" && list[i + 1].opcode == OpCodes.Brtrue)
                {
                    index = i + 2;
                }

                if (index > 0 && end < 0 && ins.opcode == OpCodes.Call && ins.operand is not null && ins.operand is MethodBase methodBase && methodBase.Name.Contains("ExecuteEvent"))
                {
                    end = i;
                }
            }
