...
internal static class StateCharge
{
    [HarmonyTranspiler]
    private static IEnumerable<CodeInstruction> Call(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
    {
        ...
        int index = list.FindIndex(ins => ins.opcode == OpCodes.Call &&
                                          ...
                                          methodBase.Name.Contains("ExecuteEvent")) - 2;

        if (index < 0)
        {
            Log.Error($"Creating Patch error: <SCPs> {{Scp096}} [StateCharge]: Index - {index} < 0");
        ...
        [
            ...
            new CodeInstruction(OpCodes.Ldc_I4_1), // 1 [Scp096State]
            ...
            new CodeInstruction(OpCodes.Callvirt,
                AccessTools.PropertyGetter(typeof(Scp096SetStateEvent), nameof(Scp096SetStateEvent.IsAllowed))),
