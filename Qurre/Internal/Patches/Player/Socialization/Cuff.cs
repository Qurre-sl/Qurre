...
namespace Qurre.Internal.Patches.Player.Socialization
{
    ...
    internal static class Cuff
    {
        ...
        private static IEnumerable<CodeInstruction> Call(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            int uncuffIndex = list.FindIndex(
                                  ins => ins.opcode == OpCodes.Call && ins.operand is not null && ins.operand is MethodBase methodBase && methodBase.Name == nameof(DisarmedPlayers.SetDisarmedStatus))
                              - 4;
            ...
            list.InsertRange(
                uncuffIndex, new[]
                {
                    ...
                    new (OpCodes.Ldfld, AccessTools.Field(typeof(DisarmMessage), nameof(DisarmMessage.PlayerToDisarm))), // target [ReferenceHub]
            ...
            int cuffIndex = list.FindLastIndex(
                                ins => ins.opcode == OpCodes.Call && ins.operand is not null && ins.operand is MethodBase methodBase && methodBase.Name == nameof(DisarmedPlayers.SetDisarmedStatus))
                            - 5;
            ...
            list.InsertRange(
                cuffIndex, new[]
                {
                    ...
                    new CodeInstruction(OpCodes.Callvirt, AccessTools.PropertyGetter(typeof(API.Player), nameof(API.Player.ReferenceHub))),
