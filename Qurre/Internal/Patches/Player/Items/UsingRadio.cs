    ...
    internal static class UsingRadio
    {
        ...
        private static IEnumerable<CodeInstruction> Call(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            for (int i = index; i < list.Count; i++)
            {
                ...
                if (!found && ins.opcode == OpCodes.Call && ins.operand is not null && ins.operand is MethodBase methodBase && methodBase.Name == nameof(EventsManager.Loader.InvokeEvent))
                {
                    found = true;
                }
                ...
                else if (startIndex > 0 && ins.opcode == OpCodes.Call && ins.operand is not null && ins.operand is MethodBase endMethod && endMethod.Name == nameof(Mathf.Clamp01))
                {
                    endIndex = i + 2;
                }
            }
        ...
        private static float GetBattery(UsingRadioEvent @event) => Mathf.Clamp01(@event.Battery / 100 - @event.Consumption / 100);
    }
}