    ...
    internal static class DropItem
    {
        ...
        private static IEnumerable<CodeInstruction> Call(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            int index = list.FindIndex(
                            ins => ins.opcode == OpCodes.Call && ins.operand is not null && ins.operand is MethodBase methodBase && methodBase.Name == nameof(InventoryExtensions.ServerDropItem))
                        - 2;
            ...
            list.InsertRange(
                index, new[]
                {
                    ...
                    new (OpCodes.Ldfld, AccessTools.Field(typeof(Inventory), nameof(Inventory._hub))),
