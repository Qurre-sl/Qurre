...
namespace Qurre.Internal.Patches.Player.Pickups
{
    ...
    internal static class ThrowProjectile
    {
        ...
        private static IEnumerable<CodeInstruction> Call(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            int index = list.FindIndex(ins => ins.opcode == OpCodes.Call && ins.operand is not null && ins.operand is MethodBase methodBase && methodBase.Name == nameof(ThrowableItem.ServerThrow))
                        - 8;
            ...
            list.InsertRange(
                index, new[]
                {
                    ...
                    new (OpCodes.Call, AccessTools.PropertyGetter(typeof(ItemBase), nameof(ItemBase.Owner))),
