...
namespace Qurre.Internal.Patches.Player.Interact
{
    ...
    internal static class InteractGenerator
    {
        ...
        {
            yield return new (OpCodes.Ldarg_0); // Scp079Generator [instance]
            yield return new (OpCodes.Ldarg_1); // ReferenceHub [ply]
            yield return new (OpCodes.Ldarg_2); // byte [colliderId]
            yield return new (OpCodes.Call, AccessTools.Method(typeof(InteractGenerator), nameof(Invoke)));
        ...
        private static void Invoke(Scp079Generator instance, ReferenceHub ply, byte colliderId)
        {
            try
            {
                if (instance._cooldownStopwatch.IsRunning && instance._cooldownStopwatch.Elapsed.TotalSeconds < instance._targetCooldown)
                {
                    return;
                }
                ...
                switch (colliderId)
                {
                    case 0: // Open, Close, Unlock
                    {
                        if (instance.HasFlag(instance._flags, Scp079Generator.GeneratorFlags.Unlocked))
                        {
                            ...
                            InteractGeneratorEvent ev = new (
                                pl, instance.GetGenerator(),
                        ...
                        else
                        {
                            ...
                            if (ev.Allowed)
                            {
                                ...
                                instance.ServerGrantTicketsConditionally(new (ply), 0.5f);
                            }
                            else
                            {
                    ...
                    case 1: // Activate / Disable
                        if ((ply.IsHuman() || instance.Activating) && !instance.Engaged)
                        {
                            ...
                            else
                            {
                                ...
                                instance._lastActivator = new (ply);
                            }
